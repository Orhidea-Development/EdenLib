...
import dev.orhidea.edenlib.gui.IGuiClick;
import dev.orhidea.edenlib.gui.configuration.GuiDesign;
import dev.orhidea.edenlib.item.ItemStackWrapper;
import dev.orhidea.edenlib.util.InvUtil;
import dev.orhidea.edenlib.util.NumberUtil;
...
public abstract class PageGui<T> extends RefreshGui implements IClickableGui {
    ...
    public void refresh() {
        ...
        for(int i = startIndex; i < startIndex+this.amountPerPage; i++) {
            ...
            if(realSlot == -1) {
                EdenLib.get().log("Issue loading page; item listing slot is -1");
                break;
            }
