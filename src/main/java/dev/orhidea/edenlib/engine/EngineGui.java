...
import dev.orhidea.edenlib.gui.BaseGui;
...
public class EngineGui extends Engine {
    ...
    public void onClick(InventoryClickEvent event) {
        ...
        if(now - lastClick >= Conf.get().guiClickThrottleDelayMs) {
        ...
        if (InventoryUtil.isBottomInventory(event)) {
            ...
            if(!hasBottomClick && !isAllowed) {
                MixinMessage.get().msgOne(player, Conf.get().msgGuiLeaveWindowToEdit);
            }
