package dev.orhidea.edenlib.engine;

import com.massivecraft.massivecore.Engine;
import com.massivecraft.massivecore.util.InventoryUtil;
import com.massivecraft.massivecore.util.MUtil;
import dev.orhidea.edenlib.event.ArmorEquipEvent;
import dev.orhidea.edenlib.object.ArmorType;
...
public class EngineArmorEquip extends Engine {

    private static final EngineArmorEquip i = new EngineArmorEquip();
    public static EngineArmorEquip get() { return i; }

    private final List<String> blockedMaterials = MUtil.list(
    ...
    public final void inventoryClick(final InventoryClickEvent e){
        ...
        if(shift){
            ...
            if(newArmorType != null){
                ...
                if(newArmorType.equals(ArmorType.HELMET) && (equipping ? InventoryUtil.isNothing(e.getWhoClicked().getInventory().getHelmet()) : InventoryUtil.isSomething(e.getWhoClicked().getInventory().getHelmet())) || newArmorType.equals(ArmorType.CHESTPLATE) && (equipping ? InventoryUtil.isNothing(e.getWhoClicked().getInventory().getChestplate()) : InventoryUtil.isSomething(e.getWhoClicked().getInventory().getChestplate())) || newArmorType.equals(ArmorType.LEGGINGS) && (equipping ? InventoryUtil.isNothing(e.getWhoClicked().getInventory().getLeggings()) : InventoryUtil.isSomething(e.getWhoClicked().getInventory().getLeggings())) || newArmorType.equals(ArmorType.BOOTS) && (equipping ? InventoryUtil.isNothing(e.getWhoClicked().getInventory().getBoots()) : InventoryUtil.isSomething(e.getWhoClicked().getInventory().getBoots()))){
            ...
            if(numberkey){
                if(e.getClickedInventory().getType().equals(InventoryType.PLAYER)){// Prevents shit in the 2by2 crafting
                    ...
                    if(InventoryUtil.isSomething(hotbarItem)){// Equipping
                        newArmorType = ArmorType.matchType(hotbarItem);
                        newArmorPiece = hotbarItem;
                        oldArmorPiece = e.getClickedInventory().getItem(e.getSlot());
                    }else{// Unequipping
                        newArmorType = ArmorType.matchType(InventoryUtil.isSomething(e.getCurrentItem()) ? e.getCurrentItem() : e.getCursor());
                    }
                }
            }else{
                if(InventoryUtil.isNothing(e.getCursor()) && InventoryUtil.isSomething(e.getCurrentItem())){// unequip with no new item going into the slot.
                    newArmorType = ArmorType.matchType(e.getCurrentItem());
                }
    ...
    public void playerInteractEvent(PlayerInteractEvent e){
        ...
        if(e.getAction() == Action.RIGHT_CLICK_AIR || e.getAction() == Action.RIGHT_CLICK_BLOCK){
            ...
            if(newArmorType != null){
                if(newArmorType.equals(ArmorType.HELMET) && InventoryUtil.isNothing(e.getPlayer().getInventory().getHelmet()) || newArmorType.equals(ArmorType.CHESTPLATE) && InventoryUtil.isNothing(e.getPlayer().getInventory().getChestplate()) || newArmorType.equals(ArmorType.LEGGINGS) && InventoryUtil.isNothing(e.getPlayer().getInventory().getLeggings()) || newArmorType.equals(ArmorType.BOOTS) && InventoryUtil.isNothing(e.getPlayer().getInventory().getBoots())){
    ...
    public void playerDeathEvent(PlayerDeathEvent e){
        ...
        for(ItemStack i : p.getInventory().getArmorContents()){
            if(InventoryUtil.isSomething(i)){
